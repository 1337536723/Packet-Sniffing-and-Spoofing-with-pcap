...
这个项目算是网络安全中比较基础的一个入门项目。因为包在传输过程中需要经过若干路由器，所以我们可以监听所有经过的数据包，从而嗅探出包里面的信息。而借助raw socket（原始套接字）编程，我们可以仿冒包（伪造IP地址、端口等信息），从而进行欺骗。
...
不可以，必须少于帧的最大长度（1500bytes）。

注意！如果只在填充header时修改total length是不会生效的，因为sendto函数中还有一个参数是描述包的总长度的，如果没有修改这个参数的值，在发送时header中的total length字段会被参数值覆盖。如果我们同时修改了这两个地方，改动是会生效的，**只要包的长度不超出限制，我们可以设置长度为任意值，多出的部分会全部以0x00来填充**。

![length](https://raw.githubusercontent.com/familyld/Packet-Sniffing-and-Spoofing-with-pcap/master/graph/image44.png)

如果超出了帧的最大长度，则程序会在执行到sendto函数时报错：

![length](https://raw.githubusercontent.com/familyld/Packet-Sniffing-and-Spoofing-with-pcap/master/graph/image45.png)

#### 使用raw socket编程，需要为IP头计算校验码吗？
...
因为项目中我们要自己对IP包头部进行设定，所以开启了IP_HDRINCL参数，但IP包的校验码还是内核自己管，我们不算也没关系。实测确实不算也行，抓到的包校验和是正确的（和之前算了的相比）。
